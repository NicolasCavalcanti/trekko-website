    ...
    const modal = createModal('cadastro', 'Cadastrar na Trekko', `
        <form id="registerForm" onsubmit="handleRegister(event)">
            ...
            <div class="form-group">
                ...
                <small style="color: #666; font-size: 0.8em;">Mínimo 8 caracteres, incluindo maiúscula, minúscula e número</small>
            </div>
            <div class="form-group">
                <label for="registerUserType">Tipo de Usuário</label>
                <select id="registerUserType" required onchange="toggleCadasturField()">
                    <option value="">Selecione...</option>
                    <option value="trekker">🥾 Trekker (Usuário)</option>
                    <option value="guia">🧭 Guia Profissional</option>
                </select>
            </div>
            <div id="cadasturSection" class="form-group" style="display: none;">
                <label for="cadasturNumber" style="color: #dc2626;">Número CADASTUR *</label>
                <input type="text" id="cadasturNumber" placeholder="Ex: 12345678901" oninput="formatAndValidateCadastur(this)">
                <small style="color: #666; font-size: 0.8em;">
                    CADASTUR é o registro obrigatório para guias de turismo no Brasil. 
                    <a href="https://cadastur.turismo.gov.br/" target="_blank" style="color: #2D6A4F;">Saiba mais</a>
                </small>
                <div id="cadasturValidation" style="margin-top: 0.5rem; font-size: 0.8em;"></div>
            </div>
...
// CADASTUR field control functions
function toggleCadasturField() {
...
function formatAndValidateCadastur(input) {
    ...
    if (value === '') {
        validationDiv.innerHTML = '<span style="color: #dc2626;">⚠️ Número CADASTUR é obrigatório para guias</span>';
        return false;
    }
    
    if (value.length < 11) {
        validationDiv.innerHTML = '<span style="color: #f59e0b;">⚠️ CADASTUR deve ter 11 dígitos</span>';
        return false;
    }
    
    if (value.length === 11) {
        validationDiv.innerHTML = '<span style="color: #10b981;">✅ Formato válido</span>';
        return true;
    }
...
async function handleRegister(event) {
    ...
    if (!validatePassword(password)) {
        alert('Senha deve ter pelo menos 8 caracteres, incluindo maiúscula, minúscula e número');
        return;
    }
    ...
    if (userType === 'guia') {
        if (!cadastur || cadastur.trim() === '') {
            alert('Número CADASTUR é obrigatório para guias profissionais');
            return;
        }
        
        // Only proceed if formatAndValidateCadastur returns true
        if (!formatAndValidateCadastur(document.getElementById('cadasturNumber'))) {
            // The formatAndValidateCadastur function already displays a message
            return;
        }
    }
    ...
    try {
        ...
        // Add CADASTUR for guides
        if (userType === 'guia' && cadastur) {
            requestData.cadastur = cadastur;
        }
